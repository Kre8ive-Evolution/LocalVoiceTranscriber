...
class AppDelegate: NSObject, NSApplicationDelegate, AudioTranscriptionManagerDelegate, GeminiAudioRecordingManagerDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // Create menu
        let menu = NSMenu()
        menu.addItem(NSMenuItem(title: "Recording: Double-tap Ctrl (or Cmd+Opt+Z)", action: nil, keyEquivalent: ""))
        ...
        // Set up double-tap Ctrl detection (like LocalVoiceTranscriber)
        DoubleTapCtrlDetector.shared.onDoubleTapCtrl = { [weak self] in
            guard let self = self else { return }
            if !self.audioManager.isRecording && !self.geminiAudioManager.isRecording && !self.screenRecorder.recording {
                print("🎤 Starting recording (2x Ctrl)")
                self.audioManager.toggleRecording()
            }
        }
        DoubleTapCtrlDetector.shared.onSingleCtrl = { [weak self] in
            guard let self = self else { return }
            if self.audioManager.isRecording {
                print("🛑 Stopping recording (1x Ctrl)")
                self.audioManager.toggleRecording()
            }
        }
        DoubleTapCtrlDetector.shared.start()
        print("✅ Double-tap Ctrl detector ready (2x Ctrl = Start, 1x Ctrl = Stop)")
    ...
    func pasteTextAtCursor(_ text: String) {
        ...
        // Set our text to clipboard (keep it there!)
        pasteboard.clearContents()
        pasteboard.setString(text, forType: .string)
        print("📋 Text in Zwischenablage: \(text.prefix(50))...")

        // Try to paste with Cmd+V
        let source = CGEventSource(stateID: .hidSystemState)
        ...
        print("✅ Paste command sent (text stays in clipboard)")
    }
    ...
    func transcriptionDidComplete(text: String) {
        ...
        WaveformOverlay.shared.hide()

        // Check for webhook keywords (Nexa/Bruce)
        if let (keyword, url) = WebhookHandler.shared.checkForKeyword(text) {
            print("🎯 Keyword '\(keyword)' detected - sending to webhook")

            // Show processing indicator
            if let button = statusItem.button {
                button.image = NSImage(systemSymbolName: "arrow.up.circle", accessibilityDescription: "Sending...")
            }

            WebhookHandler.shared.sendToWebhook(text: text, url: url) { [weak self] result in
                DispatchQueue.main.async {
                    // Reset icon
                    if let button = self?.statusItem.button {
                        button.image = NSImage(systemSymbolName: "waveform", accessibilityDescription: "Voice Assistant")
                    ...
                    case .success(let response):
                        ...
                        let notification = NSUserNotification()
                        notification.title = "\(keyword.capitalized) antwortet"
                        notification.informativeText = String(response.prefix(150))
                        NSUserNotificationCenter.default.deliver(notification)

                        // Optionally paste response
                        self?.pasteTextAtCursor(response)

                    case .failure(let error):
                        let notification = NSUserNotification()
                        notification.title = "Webhook Fehler"
            ...
            // Normal transcription - paste text
